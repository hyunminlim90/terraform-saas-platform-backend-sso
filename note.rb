                ...
                (가) 
                    (1) TLS 구성.
                        ...
                        » (MySQL) ⇢ 마킹 값 전체 치환 ⭐
                            ...
                            (로컬 정적 호스트 매핑) ⇢ mac(build), vm-java(connect-j)
                                ...
                                ✝︎ (호스트 매핑)
                                    ...
                                    192.168.35.79       sso.private.mysql
                            ...
                            (인증서)
                                ...
                                ✝︎ (TLS)
                                    ...
                                    » (커맨드)
                                        ...
                                        (mac)
                                            ...
                                            sudo rsync -avz --no-owner --no-group --progress \
                                                ~/Desktop/opentofu/certificate/mysql/sso/ca.crt.pem \
                                                ~/Desktop/opentofu/certificate/mysql/sso/sso.private.mysql.crt.pem \
                                                ~/Desktop/opentofu/certificate/mysql/sso/sso.private.mysql.key.pem \
                                                sysadmin@mysql.opentofu.click:/home/sysadmin/cert/;

                                        (vm-mysql)

                                            mkdir -p ~/cert/sso && \
                                            mv ~/cert/ca.crt.pem \
                                                ~/cert/sso.private.mysql.crt.pem \
                                                ~/cert/sso.private.mysql.key.pem \
                                                ~/cert/sso/;

                                            sudo chmod 644 ~/cert/sso/ca.crt.pem;
                                            sudo chmod 644 ~/cert/sso/sso.private.mysql.crt.pem;
                                            sudo chmod 600 ~/cert/sso/sso.private.mysql.key.pem;
                                            sudo chown root:root ~/cert/sso/sso.private.mysql.key.pem;
                                            ls -l ~/cert/sso/ca.crt.pem;
                                            ls -l ~/cert/sso/sso.private.mysql.crt.pem;
                                            ls -l ~/cert/sso/sso.private.mysql.key.pem;
                                            mkdir -p ~/config/sso;
                                            cp ~/config/common/mysql-client-tls.cnf ~/config/sso/;
                                            cat ~/config/sso/mysql-client-tls.cnf;

                                            cat > ~/config/sso/mysql-server-tls.cnf
                                ...
                                ✝︎ (컨테이너)
                                    ...
                                    » (스펙 정의)
                                        ...
                                        (템플릿)
                                            ...
                                            mysql-sso:
                                                ...
                                                restart: always
                                                command:
                                                    - --max-connections=200 # 프로덕션 적용 검토
                                                ...
                                                ports:
                                                    - "{{__MYSQL_포트__}}:3306"
                                                environment:
                                                    - MYSQL_DATABASE=awscloud_{{__스네이크케이스_AWS제외__}}
                                                    # - MYSQL_DATABASE=sso
                                                    # - MYSQL_DATABASE=sts
                                                    - MYSQL_ROOT_PASSWORD=Mysql1234!!
                                                    - TZ=Asia/Seoul
                                                volumes:
                                                    - /home/sysadmin/mysql/mysql-sso:/var/lib/mysql
                                                    - /home/sysadmin/cert/sso/ca.crt.pem:/etc/pki/tls/certs/ca.crt.pem:ro
                                                    - /home/sysadmin/cert/sso/sso.private.mysql.crt.pem:/etc/pki/tls/certs/sso.private.mysql.crt.pem:ro
                                                    - /home/sysadmin/cert/sso/sso.private.mysql.key.pem:/etc/pki/tls/private/sso.private.mysql.key.pem:ro
                                                    - /home/sysadmin/config/sso:/etc/my.cnf.d:ro
                                                networks:
                                                - cmpa-network
                                    ...
                                    (vm-mysql)
                                        ...
                                        docker compose -f /home/sysadmin/yaml/mysql.yaml up -d

                                        docker logs -f mysql-sso
                            ...
                            (사용자 구성)
                                ...
                                ✝︎ (MySQL Workbench)
                                    ...
                                    GRANT ALL PRIVILEGES ON sso.* TO 'sysadmin'@'%'; ⇢ sso
                                    GRANT ALL PRIVILEGES ON sts.* TO 'sysadmin'@'%'; ⇢ sts

                                ✝︎ (mac)

                                    mysql -usysadmin -p'Sysadmin1234!!' -h sso.private.mysql -P{{__MYSQL_포트__}} -e status

                                ✨ MySQL Workbench sysadmin 로그인.
                        ...
                        » (Java)  ⇢ 마킹 값 전체 치환 ⭐
                            ...
                            » (PKIX / X.509 Trust Model) ⇢ 서버 신원 검증 [🔗 6.3]
                                ...
                                (CA 신뢰 등록)
                                    ...
                                    ✨ sso 제외하고 sts 부터 서버 신원 검증 플래그를 지정한다.
                ...
                (나)
                    ...
                    (2) MySQL 루트 사용자 관리.
                        ...
                        (mac)
                            ...
                            mysql -uroot -p'Mysql1234!!' -h sso.private.mysql -P{{__MYSQL_포트__}}
                ...
                (나) 
                    (1) 배포.
                        ...
                        ✝︎ (커맨드)
                            ...
                            mvn clean install;
                            sudo rsync -avz --no-owner --no-group --progress ./target/sso-0.0.1-SNAPSHOT.jar sysadmin@java.opentofu.click:/home/sysadmin/deploy/target/;
                            rm -rf ./target/sso-0.0.1-SNAPSHOT.jar;
                            
                            ls -al ~/.m2/repository/click/opentofu/sso/0.0.1-SNAPSHOT/sso-0.0.1-SNAPSHOT.jar;
                            rm -rf ~/.m2/repository/click/opentofu/sso/0.0.1-SNAPSHOT/sso-0.0.1-SNAPSHOT.jar;
                ...
                (다) 
                    (1) 프로세스 실행.
                        ...
                        ✝︎ (커맨드)
                            ...
                            nohup java -jar ~/deploy/target/sso-0.0.1-SNAPSHOT.jar > ~/deploy/nohup-logs/sso-output.log 2>&1 &
                            nohup java -Djavax.net.ssl.trustStore=/opt/app/certs/truststore.p12 -Djavax.net.ssl.trustStorePassword=changeit -jar ~/deploy/target/sso-0.0.1-SNAPSHOT.jar > ~/deploy/nohup-logs/sso-output.log 2>&1 &
                    ...
                    (2) 프로세스 포트 조회.
                        ...
                        ✝︎ (커맨드)
                            ...
                            tail -f -n 100 ~/deploy/nohup-logs/sso-output.log

                    (3) 테이블 조회.

                        ✝︎ (MySQL Workbench)

                            ✨ {{__스네이크케이스_AWS제외__}}_entify 확인.
